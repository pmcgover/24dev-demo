...
min(avg_field_cuttings_ft) AS avg_field_cuttings_ft,
...
----------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------
-- Begin Annual Nursery Views - See instructions at the end of this file:
----------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------
...
trunc(avg(sum_nursery_cuttings_ft),0) as avg_nursery_cuttings_ft,
...
-- Refactor rank with; DENSE_RANK() OVER (ORDER BY cut_fcut_reps DESC

------------------------------------------------------------------------------
-- Annual Nursery Process below:
------------------------------------------------------------------------------
-- Annual Nursery process to enroll new test_detail data into r4st after Fall Nursery data entry:
...
--
-- Once stock is lifted and entered into test_detail spreadsheet: 
-- Re-order all nusery records in ascending key order.
-- Assign desireable clones/families with the applicable "selection_type" character.
-- Assign 'Y" to Is_plus_ynu records intended for promotion to next year's nursery and is a selection for a future regional field trial.  
-- Assign 'R' to "Retry" all good materials that need to be retested again.
-- Assign 'D" to discard everything that will not be planted in nursery next year. 
------------------------------------------------------------------------------
------------------------------------------------------------------------------
